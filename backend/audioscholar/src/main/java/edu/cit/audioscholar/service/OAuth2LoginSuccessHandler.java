...
public class OAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if (providerId == null) {
				logger.error("Could not extract required providerId from provider '{}'. Attributes: {}", provider,
						attributes);
				response.sendRedirect("/login?error=provider_info_missing");
				return;
			}
			if (email == null) {
				logger.warn("Email not provided by OAuth2 provider '{}' for providerId {}. Proceeding without email.",
						provider, providerId);
			}

			logger.info("OAuth2 login success. Provider: {}, ProviderId: {}, Email: {}, Name: {}", provider, providerId,
					email, name);
	...
	private void processUserLogin(String email, String name, String pictureUrl, String provider, String providerId)
		...
		if (user != null) {
			...
			if (user.getProvider() == null || !provider.equals(user.getProvider())) {
				logger.warn("Updating provider for user {} from {} to {}", user.getUserId(), user.getProvider(),
						provider);
