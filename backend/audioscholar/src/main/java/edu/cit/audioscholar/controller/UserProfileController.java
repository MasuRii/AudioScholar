...
public class UserProfileController {
	...
	public ResponseEntity<?> uploadUserAvatar(@RequestParam("avatar") MultipartFile file) {
		...
		if (contentType == null || !ALLOWED_IMAGE_TYPES.contains(contentType.toLowerCase())) {
			log.warn("Avatar upload failed for user {}: Invalid file type '{}'. Allowed types: {}", userId, contentType,
					ALLOWED_IMAGE_TYPES);
		...
		if (file.getSize() > MAX_AVATAR_SIZE_BYTES) {
			log.warn("Avatar upload failed for user {}: File size {} exceeds limit of {} bytes.", userId,
					file.getSize(), MAX_AVATAR_SIZE_BYTES);
			return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE)
					.body("Avatar file size exceeds the limit (" + (MAX_AVATAR_SIZE_BYTES / 1024 / 1024) + "MB).");
		}

		try {
			log.info("Processing valid avatar upload for file: {} from user: {}", file.getOriginalFilename(), userId);
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("Error updating user profile after avatar upload.");
		} catch (RuntimeException e) {
			if (e.getMessage() != null && e.getMessage().startsWith("User not found")) {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("An unexpected error occurred during avatar upload.");
		} catch (Exception e) {
	...
	private String getUserIdFromAuthentication(Authentication authentication) {
		...
		if (userId == null || userId.isBlank()) {
			log.error("Could not extract userId (subject) from JWT token for principal name: {}",
