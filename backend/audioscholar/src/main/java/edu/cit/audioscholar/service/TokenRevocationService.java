	...
	public void revokeToken(String token) {
		...
		try {
			...
			if (cache != null) {
				...
				log.warn("Cache '{}' not found. Could not cache revoked token jti: {}", CACHE_NAME, jti);
			}

		} catch (ExpiredJwtException e) {
			log.debug("Attempted to revoke an already expired token (jti: {}).", e.getClaims().getId());
		} catch (FirestoreInteractionException e) {
			log.error("Firestore error while adding token jti {} to denylist: {}", jti, e.getMessage(), e);
		} catch (JwtException e) {
			log.error("Error processing JWT during revocation: {}", e.getMessage());
		} catch (Exception e) {
			log.error("Unexpected error while revoking token (jti: {}): {}", jti, e.getMessage(), e);
		}
	}
		...
		try {
			...
			if (jwtDenylistCache != null) {
				...
				if (cachedResult != null) {
					log.debug("Token jti {} found in cache '{}'. Revoked: {}", tokenId, CACHE_NAME, cachedResult);
					return cachedResult;
				}
				log.debug("Token jti {} not found in cache '{}'. Checking Firestore.", tokenId, CACHE_NAME);
			}

			try {
				Map<String, Object> denylistEntry = firebaseService.getData(DENYLIST_COLLECTION, tokenId);
				...
				log.error("Error checking Firestore denylist for token jti {}: {}. Allowing access.", tokenId,
						e.getMessage());
				return false;
			}
		} catch (Exception e) {
