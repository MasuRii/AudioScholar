		...
		if (authentication != null && authentication.isAuthenticated()) {
			...
			if (token != null) {
				...
				if (tokenRevocationService.isTokenRevoked(token)) {
					...
					response.getWriter()
							.write("{\"error\": \"Unauthorized\", \"message\": \"Token has been revoked.\"}");
				...
				log.debug(
						"User is authenticated but no Bearer token found in request? This might indicate session-based auth leaking through.");
			}
		} else {
			log.debug("No authenticated user found in context, skipping denylist check for path: {}",
