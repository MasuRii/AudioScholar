	...
	public List<Monitor> getMonitors() {
		...
		try {
			...
			UptimeRobotResponse response = client.post().uri(GET_MONITORS_PATH)
					.body(BodyInserters.fromFormData(formData)).retrieve().bodyToMono(UptimeRobotResponse.class)
					.timeout(API_TIMEOUT).onErrorResume(TimeoutException.class, ex -> {
						log.warn("UptimeRobot API call timed out after {}. Returning empty list.", API_TIMEOUT);
			...
			if (response != null && "ok".equalsIgnoreCase(response.getStat())) {
				...
				return response.getMonitors() != null ? response.getMonitors() : Collections.emptyList();
			} else if (response == null) {
				log.warn("UptimeRobot API call did not return a response (likely due to timeout).");
				return Collections.emptyList();
			} else {
				log.error("Failed to get monitors from UptimeRobot. Status: {}, Response: {}", response.getStat(),
						response);
				return Collections.emptyList();
			}
		} catch (Exception e) {
			log.error("Error calling UptimeRobot API at {}{}: {}", apiBaseUrl, GET_MONITORS_PATH, e.getMessage(), e);
			return Collections.emptyList();
		}
	}
}
