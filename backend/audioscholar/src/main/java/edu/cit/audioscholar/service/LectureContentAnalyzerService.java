...
public class LectureContentAnalyzerService {
	...
	public AnalysisResults analyzeLectureContent(String recordingId) {
		...
		try {
			...
			log.info("Successfully retrieved {} topics from summary for recording ID: {}", topics.size(), recordingId);
			return new AnalysisResults(topics);

		} catch (ExecutionException | InterruptedException e) {
			log.error("Failed to retrieve summary data from Firestore for recording ID: {}", recordingId, e);
			Thread.currentThread().interrupt();
			return new AnalysisResults(Collections.emptyList());
		} catch (Exception e) {
			log.error("Unexpected error during content analysis (topic retrieval) for recording ID: {}", recordingId,
					e);
			return new AnalysisResults(Collections.emptyList());
		}
	}
