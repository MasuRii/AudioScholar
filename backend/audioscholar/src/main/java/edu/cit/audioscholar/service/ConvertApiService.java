...
public class ConvertApiService {
	...
	public String convertPptxUrlToPdfUrl(String pptxUrl) throws Exception {
		...
		try {
			...
			logger.info("ConvertAPI conversion completed successfully. Conversion cost: {}", result.conversionCost());
			...
			throw new Exception("Error during PPTX to PDF conversion: " + e.getCause().getMessage(), e.getCause());
		} catch (TimeoutException e) {
			logger.error("PPTX to PDF conversion timed out after {} seconds", TIMEOUT_SECONDS, e);
			throw new Exception("PPTX to PDF conversion timed out after " + TIMEOUT_SECONDS + " seconds", e);
		} catch (Exception e) {
			logger.error("Unexpected error during PPTX to PDF conversion", e);
			throw new Exception("Unexpected error during PPTX to PDF conversion: " + e.getMessage(), e);
		}
	}
}
